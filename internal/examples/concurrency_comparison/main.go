...
func main() {
	...
	// Run DCB concurrency control test
	runSingleApproach(ctx, store, *numUsers, *numSeats, *ticketsPerUser)
}

// runSingleApproach runs a single concurrency control approach
func runSingleApproach(ctx context.Context, store dcb.EventStore, numUsers, numSeats, ticketsPerUser int) {
	fmt.Printf("=== Testing %s ===\n", getConcurrencyMethod())
	results := runConcurrencyTest(ctx, store, numUsers, numSeats, ticketsPerUser)
...
func runConcurrencyTest(ctx context.Context, store dcb.EventStore, numUsers, numSeats, ticketsPerUser int) TestResults {
	// Create a concert with limited seats
	concertID := fmt.Sprintf("concert_%d_%s", time.Now().Unix(), getConcurrencyMethod())
...
func getConcurrencyMethod() string {
